...
| `SYNC-002` | Critical | The origin checkpoint is `(height, block hash, per-pool tree state)`. Its hash is pinned into the attested production binary, is non-placeholder, and must be on the selected canonical chain. | Known checkpoint vector, wrong-hash tests, and production-artifact inspection. |
