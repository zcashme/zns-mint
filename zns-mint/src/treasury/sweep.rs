...
pub enum SweepRequest {
    Sapling { notes: Vec<incrementalmerkletree::Position>, amount: Zatoshis },
    Orchard { notes: Vec<orchard::note::Rho>, amount: Zatoshis },
}

/// The Treasury cold-storage transparent address for auto-sweeps.
/// Address: t1ZqkmvGxQmANohacyq2YnyVL41jrzovkBQ
/// Script: OP_DUP OP_HASH160 af288016bfca2b4d3c97f7919c943b6e5d0a6623 OP_EQUALVERIFY OP_CHECKSIG
pub const COLD_ADDRESS_SCRIPT: [u8; 25] = [
    0x76, 0xa9, 0x14, 0xaf, 0x28, 0x80, 0x16, 0xbf, 
    0xca, 0x2b, 0x4d, 0x3c, 0x97, 0xf7, 0x91, 0x9c, 
    0x94, 0x3b, 0x6e, 0x5d, 0x0a, 0x66, 0x23, 0x88, 0xac
];
...
pub fn sweep_policy(
    ...
    // 2. Sapling Policy
    let sapling_balance: u64 = wallet.sapling_notes_for(TREASURY_ACCOUNT).map(|n| n.note.value().inner()).sum();
    if sapling_balance > SWEEP_THRESHOLD {
        ...
        if let Some((selected, _)) = crate::wallet::selection::select_sapling_funds(wallet, TREASURY_ACCOUNT, sweep_amount, &exclude) {
    ...
    // 3. Orchard Policy
    let orchard_balance: u64 = wallet.orchard_notes_for(TREASURY_ACCOUNT).map(|n| n.note.value().inner()).sum();
