...
use crate::mint::{ChainCursor, Mint, REGISTRY_ACCOUNT, TREASURY_ACCOUNT};
...
/// Runs the full boot sequence and returns the owned [`Mint`].
///
/// Every step panics on failure — boot failures are fatal. The mint refuses
/// to enter the orchestrator loop in a degraded state.
pub async fn boot() -> Mint {
    ...
    Mint::new(
        ...
        Registry::new(),
        Treasury::new(),
        ChainCursor::from_metadata(checkpoint.metadata),
        treasury_keys,
        registry_keys,
    )
}
...
async fn origin_checkpoint(rpc: &zcash::JsonRpc) -> CheckpointData {
    let checkpoint_height =
        ironwood_activation_height() - BlockHeight::from_u32(1);
