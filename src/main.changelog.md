...
- Added a `PINNED_ORIGIN_HASH` constant and an assertion that checks the fetched checkpoint's `block_hash()` against the pinned value (unless `pre-nu63-activation` is enabled), acting as a trust anchor for the commitment trees.

## 2026-07-12 (pinned ZNS origin checkpoint)

- Replaced the dynamic tip-based birthday checkpoint in `boot.rs` with a hardcoded
  `ZNS_ORIGIN_HEIGHT` (set to 3,400,000 for development).
- The boot sequence now asserts that the node tip is at or ahead of `ZNS_ORIGIN_HEIGHT - 1`.
- The orchestrator will start syncing predictably from `ZNS_ORIGIN_HEIGHT` instead of the current chain tip.
- Added a scratch `pinned_origin_checkpoint()` that returns empty tree frontiers; for
  production, this must be updated to the true verified block hash and shielded tree state.

## 2026-07-12 (OTP auth wiring)

- Renamed `auth::OtpStore` to `PendingOtps` and `OtpKey` to `ChallengeKey` to clarify their short-lived, in-memory nature. Keys now use strongly-typed `Name` and `UnifiedAddress` instead of raw strings.
- Added `auth::encode_otp_relay_memo` to construct the `ZNS:otp:<name>:<verb>:<ua>:<otp>` memo (zero-padded to 512 bytes) that Treasury sends to the controller for authorization.
- Updated `registry::authorize` to use the typed `PendingOtps` and `ChallengeKey`.

## 2026-07-12 (scanner nullifier and block-metadata handoff)

- `sync::BlockOutput` now retains the upstream `BlockMetadata` derived by
  `ScannedBlock::to_block_metadata`, so the future Mint transition can advance
  its cursor only after all local state has accepted the block.
- Every received Orchard, Sapling, and Ironwood note now carries the nullifier
  derived by the upstream scanner. `WalletBalance` inserts these into its
  per-pool nullifier indexes and removes/restores them symmetrically during
  rewind.
- Replaced the Sapling `[0; 32]` placeholder identity with the actual global
  Sapling note-commitment-tree position. This preserves distinct notes and
  makes later spend lookup reversible.
- `sync::scan_block` now mutates upstream `Nullifiers` with
  `Nullifiers::update_with` after producing the local output, preserving
  upstream sequential in-memory scan semantics for the future Mint scanner
  state.

## 2026-07-12 (Registry fee-liquidity request shape)
- Added Registry fee-note classification and a simple fixed-batch liquidity
  policy. Registry notes whose memos parse as ZNS Name Notes are never fee
  liquidity, even if their value is non-zero.
- Treasury Registry funding now triggers when the Registry has fewer than
  `MIN_REGISTRY_FEE_NOTES` fee notes and requests
  `REGISTRY_FUNDING_BATCH_SIZE` outputs of `REGISTRY_FEE_NOTE_TARGET_VALUE`
  each.
- `main.rs` still does not build the funding transaction; it logs the planned
  output shape and keeps the `build_funding_transaction` warning explicit.
...
- **No transparent output detection.** The published Treasury UA omits the
  ...
  `docs/protocol.md (§2–3)` "Treasury Capability" for the
  decision record.
...
- Updated `docs/design/08-chain-sync.md`: "Scanner Boundary" now describes
...
- Updated `docs/protocol.md (§2–3)`: "Treasury Capability"
...
- Added `docs/protocol.md` (consolidated protocol spec) and `docs/design/` (implementation design docs) as the protocol design source for the mint.
...
- `docs/design/07-mint-run-loop.md` records the target long-running loop shape
...
- Added `docs/design/12-programming-model.md` to describe the concrete
...
- Added `docs/design/13-implementation-slices.md` to break the mint into
...
- Added `docs/design/14-wallet-design.md`: in-memory wallet design (per-account
...
- `src/auth.rs` module doc + `IssuedOtp.memo` comment + `src/auth.rs.context.md`
  ...
  `crate::payload`).
- `docs/design/14-wallet-design.md`: added F10 (Treasury Policy vs OTP Relay
...
- `docs/protocol.md` (from `00-overview.md`), `docs/design/13-implementation-slices.md`:
...
- Added `docs/design/15-treasury-module.md`: full design for the
...
- `docs/design/14-wallet-design.md` F8 and F10 and Module Boundaries
...
## 2026-07-11 (boot API boundary)
- `boot::boot` remains public because integration tests and `regtest-harness` are first-class external consumers, but it now returns a named `BootedMint<E>` instead of a positional tuple.
- Attestation report persistence moved behind `BootEnv::persist_attestation_report`, keeping the generic boot sequence from choosing a filesystem path.
- Decrypted seed plaintext is zeroized after copying into `Secret<[u8; 32]>`, including the malformed-length failure path.

## 2026-07-12 (main back to boot boundary)
- Removed the unfinished catch-up/live-action loop from `main.rs`. The binary now boots, marks metrics healthy, logs the birthday height / NU6.3 state, and waits for Ctrl-C.
- This keeps orchestration out of `main.rs` until the scanner/run-loop boundary is actually implemented. The deleted loop called placeholder sync helpers and policy modules without applying `sync::BlockOutput` to wallet/registry state.

## 2026-07-12 (Zebra chain helper boundary)
- Moved Zebra tip parsing, display-hash conversion, and gRPC full-block fetch/parsing out of `sync.rs` and into the `zcash::ChainClient` boundary.
- `sync.rs` now no longer owns raw Zebra proto conversion or `GetBlock` request construction; those are node I/O concerns, not scanner concerns.

## 2026-07-12 (Mint ownership boundary)
- Introduced `mint::Mint` as the concrete owner of runtime state:
  `ChainClient`, `Wallet`, `Registry`, owned `Treasury`, `ChainCursor`, and
  both account key handles.
- Replaced the boot-specific `BootedMint` return with `Mint`; `main.rs` now
  logs the mint height through `Mint::height()`.
- Added `mint::ChainCursor` as a local semantic wrapper around upstream
  `BlockMetadata`, so scanner continuity state is named as the mint's
  fully-applied chain prefix.
- Changed `treasury::Treasury` from a borrowed `Treasury<'w>` wallet view into
  an owned newtype over Treasury policy state. Treasury still does not own
  notes; methods take `&Wallet` when they need to inspect funds.
- Removed the runtime `nu6_3_active` boolean from the mint shape. Production
