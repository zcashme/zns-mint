    ...
    'outer: loop {
        ...
        loop {
            ...
            let produced: Vec<zcash_primitives::transaction::TxId> = [
                zns_mint::mint::treasury::sweep_ironwood_to_vault(
                    &network,
                    &mut wallet,
                    &treasury_keys,
                    &sapling_spend,
                ...
                zns_mint::mint::treasury::sweep_sapling_to_vault(
                    &network,
                    &mut wallet,
                    &treasury_keys,
                    &sapling_spend,
            ...
            for txid in produced {
                match wallet.get_transaction(txid) {
                    Ok(Some(tx)) => {
                        match source.submit_transaction(&tx, sweep_tip, tip_hash).await {
