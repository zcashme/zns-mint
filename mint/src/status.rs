...
use zns_chain::{scan_mempool, ScannerConfig};
...
pub type SharedChainStatus = Arc<RwLock<ChainStatus>>;

pub fn new_shared_status() -> SharedChainStatus {
    Arc::new(RwLock::new(ChainStatus::default()))
}

pub async fn record_tick_status(
    status: &SharedChainStatus,
    registry: &Registry,
    spend: &SpendLane,
    ...
    let spendable_zat = if let Some(treasury) = treasury {
    ...
    let mempool_notes = scan_mempool(scanner, chain_tip)
        .await
    ...
    *status.write().await = ChainStatus {
        ...
        spendable_zat,
        mempool_notes,
        spend_queue_depth: spend.pending_count() as u32,
        in_flight,
        treasury_available: treasury.is_some(),
        last_poll_unix,
    };
}